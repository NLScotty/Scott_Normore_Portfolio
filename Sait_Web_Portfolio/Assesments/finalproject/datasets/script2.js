...
function search(inputValue,inputType){
    if(inputType == "number"){
      ...
      searchByURL(inputValue);
    }
      else if(inputType =="location"){
      searchByLocation(inputValue);
    }
}
	  ...
	  for(var i=0; i<data.length; i++){
        ...
        var description=camera_url.description;
		    if(description.split(" ")[1]==(inputValue)){	
					  ...
					  output+="</td><td>"
            output+=camera=obj.camera_url.url;
					  output+="</td><td>"
					  output+=obj.quadrant;
					  output+="</td><td>"
...
function searchByURL(inputValue){
  var tableBody=document.getElementById("tbody");
  var output="";
  for(var i=0; i<data.length; i++){
      var obj=data[i];
      var camera_url=obj.camera_url;
      var url=camera_url.url;
      if(url.includes(inputValue)){	
          output+="<tr><td>"
          output+=camera=obj.camera_url.description;
          output+="</td><td>"
          output+=camera=obj.camera_url.url;
...
function searchByLocation(inputValue){
  var tableBody=document.getElementById("tbody");
  var output="";
  for(var i=0; i<data.length; i++){
      var obj=data[i];
      var location=obj.camera_location;
      if(location.includes(inputValue)){	
          output+="<tr><td>"
          output+=camera=obj.camera_url.description;
          output+="</td><td>"
          output+=camera=obj.camera_url.url;
